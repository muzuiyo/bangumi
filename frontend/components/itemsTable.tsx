...
const ItemsTable = ({ items, emptyText = "暂无记录", isLoading, skeletonRows, isError }: Props) => {
  ...
  const renderSkeletonRows = () => {
    return Array.from({ length: skeletonRows }).map((_, index) => (
      <tr key={`skeleton-${index}`} className="skeleton-row">
        <td>
          <div className="skeleton-cell" style={{ width: "30px", margin: "0 auto"}} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "120px" }} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "60px", margin: "0 auto" }} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "60px", margin: "0 auto" }} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "50px", margin: "0 auto" }} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "200px" }} />
        </td>
        <td>
          <div className="skeleton-cell" style={{ width: "90px", margin: "0 auto" }} />
        </td>
      </tr>
    ));
  };
  ...
  return (
    <>
                ...
                renderSkeletonRows()
                  ...
                  items.map((row, index) => (
                    <tr key={`${row.id}-${index}`}>
                      ...
                      <td>
                        <span
                          className={`badge ${typeClassMap[row.mediaType] ?? "badge-neutral"}`}
                        >
                          {row.mediaType}
                        </span>
                      </td>
                      <td>
                        <span
                          className={`badge ${statusClassMap[row.status] ?? "badge-neutral"}`}
                        >
                          {row.status}
                        </span>
                      </td>
                      ...
                      <td>
                        {row.comment ? renderCommentWithLinks(row.comment) : "N/A"}
                      </td>
                      <td>{row.updatedAt}</td>
                    </tr>
                  ))
      ...
      {activeItem ? (
        <InfoCard item={activeItem} onClose={() => setActiveItem(null)} />
      ) : null}
    </>
  );
};
